// Missile system requirements data

// System requirements
export const systemRequirements = [
  {
    id: "SR-MISS-001",
    title: "Range Capability",
    description: "The missile system shall have a minimum range of 250 kilometers.",
    priority: "Critical",
    status: "Verified",
    category: "Performance",
    createdBy: "John Smith",
    createdDate: "2023-01-15",
    updatedBy: "Maria Johnson",
    updatedDate: "2023-06-28",
    version: "1.2",
    tags: ["Range", "Performance", "Critical"],
    verificationMethod: "Test"
  },
  {
    id: "SR-MISS-002",
    title: "Guidance Accuracy",
    description: "The missile guidance system shall achieve a CEP (Circular Error Probable) of less than 2.5 meters.",
    priority: "Critical",
    status: "In Progress",
    category: "Guidance",
    createdBy: "Maria Johnson",
    createdDate: "2023-01-20",
    updatedBy: null,
    updatedDate: null,
    version: "1.0",
    tags: ["Guidance", "Accuracy"],
    verificationMethod: "Test"
  },
  {
    id: "SR-MISS-003",
    title: "Environmental Operating Temperature",
    description: "The missile system shall operate within temperature range of -40°C to +50°C.",
    priority: "High",
    status: "Verified",
    category: "Environmental",
    createdBy: "Robert Chen",
    createdDate: "2023-02-05",
    updatedBy: "John Smith",
    updatedDate: "2023-05-12",
    version: "1.1",
    tags: ["Environmental", "Temperature"],
    verificationMethod: "Test"
  },
  {
    id: "SR-MISS-004",
    title: "Warhead Effectiveness",
    description: "The missile warhead shall be capable of neutralizing hardened targets with reinforced concrete up to 2 meters thick.",
    priority: "Critical",
    status: "In Progress",
    category: "Lethality",
    createdBy: "Alex Rodriguez",
    createdDate: "2023-01-25",
    updatedBy: "John Smith",
    updatedDate: "2023-04-10",
    version: "1.1",
    tags: ["Warhead", "Lethality", "Penetration"],
    verificationMethod: "Test"
  },
  {
    id: "SR-MISS-005",
    title: "Launch Platform Integration",
    description: "The missile shall be compatible with air, sea, and land-based launch platforms.",
    priority: "High",
    status: "In Progress",
    category: "Integration",
    createdBy: "Sarah Miller",
    createdDate: "2023-02-10",
    updatedBy: null,
    updatedDate: null,
    version: "1.0",
    tags: ["Integration", "Platform", "Compatibility"],
    verificationMethod: "Test"
  }
];

// Functional requirements
export const functionalRequirements = [
  {
    id: "FR-MISS-001",
    title: "Target Acquisition",
    description: "The missile shall be able to acquire targets at a distance of at least 200 kilometers.",
    systemRequirement: "SR-MISS-001",
    priority: "Critical",
    status: "Verified",
    category: "Targeting",
    functionality: "Target Detection",
    inputs: ["Radar Signal", "Infrared Signal"],
    outputs: ["Target Location Data"],
    constraints: "Must operate in all weather conditions",
    createdBy: "David Park",
    createdDate: "2023-01-25",
    updatedBy: "Sarah Miller",
    updatedDate: "2023-06-15",
    version: "1.3",
    dependsOn: [],
    verificationCriteria: "Successfully detect 95% of targets at specified distance",
    tags: ["Targeting", "Detection", "Radar"]
  },
  {
    id: "FR-MISS-002",
    title: "Guidance Control",
    description: "The missile shall continuously adjust flight path based on target tracking data.",
    systemRequirement: "SR-MISS-002",
    priority: "Critical",
    status: "In Progress",
    category: "Guidance",
    functionality: "Flight Control",
    inputs: ["Target Location Data", "Missile Position Data"],
    outputs: ["Control Surface Commands"],
    constraints: "Adjustments must be made within 10ms of receiving data",
    createdBy: "Sarah Miller",
    createdDate: "2023-02-05",
    updatedBy: null,
    updatedDate: null,
    version: "1.0",
    dependsOn: ["FR-MISS-001"],
    verificationCriteria: "Maintain tracking lock on target throughout flight",
    tags: ["Guidance", "Control", "Flight Path"]
  },
  {
    id: "FR-MISS-003",
    title: "Thermal Management",
    description: "The thermal management system shall maintain internal electronic component temperatures within operational limits.",
    systemRequirement: "SR-MISS-003",
    priority: "High",
    status: "Verified",
    category: "Environmental",
    functionality: "Temperature Control",
    inputs: ["Internal Temperature Sensors", "External Temperature Sensors"],
    outputs: ["Cooling System Commands"],
    constraints: "Must maintain electronics below 85°C in all operational environments",
    createdBy: "Robert Chen",
    createdDate: "2023-02-15",
    updatedBy: "John Smith",
    updatedDate: "2023-05-20",
    version: "1.1",
    dependsOn: [],
    verificationCriteria: "Electronics remain within operational temperature limits during all test scenarios",
    tags: ["Thermal", "Temperature", "Electronics"]
  },
  {
    id: "FR-MISS-004",
    title: "Warhead Detonation",
    description: "The warhead shall detonate at the optimal point to maximize effectiveness against the target.",
    systemRequirement: "SR-MISS-004",
    priority: "Critical",
    status: "In Progress",
    category: "Lethality",
    functionality: "Detonation Control",
    inputs: ["Target Material Data", "Distance to Target", "Relative Velocity"],
    outputs: ["Detonation Signal"],
    constraints: "Must determine optimal detonation timing within 1ms",
    createdBy: "Alex Rodriguez",
    createdDate: "2023-02-10",
    updatedBy: "Maria Johnson",
    updatedDate: "2023-04-25",
    version: "1.1",
    dependsOn: ["FR-MISS-002"],
    verificationCriteria: "Warhead detonates within optimal effectiveness window",
    tags: ["Warhead", "Detonation", "Timing"]
  }
];

// Test cases
export const testCases = [
  {
    id: "TC-MISS-001",
    title: "Maximum Range Test",
    description: "Verify the missile can reach targets at maximum specified range.",
    requirements: ["SR-MISS-001", "FR-MISS-001"],
    status: "Passed",
    priority: "Critical",
    createdBy: "Test Team Alpha",
    createdDate: "2023-03-10",
    executionDate: "2023-04-15",
    testSteps: [
      "Launch missile at maximum range target",
      "Track missile trajectory",
      "Verify target acquisition and hit"
    ],
    expectedResults: "Missile reaches and neutralizes target at 250km distance",
    actualResults: "Target at 255km successfully neutralized",
    testEnvironment: "Field Range Alpha, Conditions: Clear Weather"
  },
  {
    id: "TC-MISS-002",
    title: "Environmental Temperature Test - Cold",
    description: "Verify missile operation at minimum temperature.",
    requirements: ["SR-MISS-003", "FR-MISS-003"],
    status: "Passed",
    priority: "High",
    createdBy: "Test Team Beta",
    createdDate: "2023-03-15",
    executionDate: "2023-04-20",
    testSteps: [
      "Condition missile in temperature chamber at -40°C",
      "Verify all systems operational",
      "Perform simulated launch and flight"
    ],
    expectedResults: "All systems function normally at -40°C",
    actualResults: "All systems functional, guidance response 2% slower than nominal",
    testEnvironment: "Environmental Test Chamber, Condition: -40°C"
  },
  {
    id: "TC-MISS-003",
    title: "Guidance Accuracy Test",
    description: "Verify guidance system achieves required accuracy.",
    requirements: ["SR-MISS-002", "FR-MISS-002"],
    status: "In Progress",
    priority: "Critical",
    createdBy: "Test Team Alpha",
    createdDate: "2023-03-20",
    executionDate: null,
    testSteps: [
      "Deploy multiple test targets at varying ranges",
      "Launch missile at each target",
      "Measure miss distance for each engagement"
    ],
    expectedResults: "CEP of less than 2.5 meters across all test engagements",
    actualResults: "Pending completion",
    testEnvironment: "Field Range Beta, Controlled Environment"
  },
  {
    id: "TC-MISS-004",
    title: "Warhead Penetration Test",
    description: "Verify warhead capability against hardened targets.",
    requirements: ["SR-MISS-004", "FR-MISS-004"],
    status: "In Progress",
    priority: "Critical",
    createdBy: "Test Team Gamma",
    createdDate: "2023-03-25",
    executionDate: null,
    testSteps: [
      "Construct test targets with 2m reinforced concrete",
      "Launch missile at targets",
      "Assess penetration and damage effects"
    ],
    expectedResults: "Successful penetration and neutralization of hardened target",
    actualResults: "Pending completion",
    testEnvironment: "Weapons Testing Range, Hardened Target Facility"
  }
];

// Combined requirements data
export default {
  systemRequirements,
  functionalRequirements,
  testCases
}; 